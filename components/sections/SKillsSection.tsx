...
function SkillsSection() {
  const skills = [
    ...
    {
      title: "Programming",
      items: ["JavaScript", "TypeScript", "PHP", "Java", "C", "C++", "SQL"],
      icon: <FaCode />,
      bgColor: "bg-purple-600",
    },
    {
      title: "Tools",
      items: ["Git", "GitHub", "Redux", "JSON Server"],
      icon: <FaTools />,
      bgColor: "bg-orange-500",
    },
  ];

  const renderSkillCards = skills.map((skill, idx) => (
    <Card key={idx}>
      <div
        className={`rounded-full ${skill.bgColor} text-white w-8 h-7 md:w-12 md:h-10 flex items-center justify-center mr-4`}
      >
        {skill.icon}
      </div>
  ...
  return (
    ...
    >
      {/* Text */}
      <div className="flex flex-col ml-0  mt-8  items-center">
        <h3 className="text-xl md:text-4xl text-gray-600 font-bold">
          My Skills
        </h3>

        <p className="text-gray-600 mt-4 max-w-lg tracking-wide leading-7 text-[12px] md:text-sm">
          I have experience working across the full stack, building modern web
          applications using frontend and backend technologies. I focus on
          writing clean, scalable code and delivering efficient solutions.
        </p>
      </div>
      {/* Cards */}
      <div className="flex flex-col md:flex-row space-y-2 space-x-4 mt-8">
        {renderSkillCards}
      </div>
    </motion.section>
  );
